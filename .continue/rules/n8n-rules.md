...
This is an n8n custom node starter repository for building community nodes with TypeScript.
...
### DECLARATIVE (for REST APIs)
- Reference: `nodes/GithubIssues/GithubIssues.node.ts`
- Uses `routing.send` for HTTP request building
- Documentation: `docs/12-declarative-routing.md`

### PROGRAMMATIC (for SDKs, complex logic)
- Reference: `nodes/Example/Example.node.ts`
- Custom `execute()` method
- Documentation: `docs/13-custom-execute-methods.md`
...
│   └── user/
│       ├── index.ts           # Resource entry + operations
